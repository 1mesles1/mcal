...
const MONTHS_RU: [&str; 12] = [
    ...
    "Июль", "Август", "Сентябрь", "Октябрь", "Ноябрь", "Декабрь",  // исправлена опечатка
];
...
impl EventDate {
    ...
    fn parse(input: &str) -> Result<Self, String> {
        ...
        let year_str = parts[2]; // убрали mut

        let full_year = if year_str.len() == 2 {
            ...
            year_str.parse::<i32>().map_err(|_| "Invalid year".to_string())?
        };
        ...
        // Monthly: день = 00, месяц != 00
        if day_str == "00" {
            let month = month_str.parse::<u32>().map_err(|_| "Invalid month".to_string())?;
            if month < 1 || month > 12 {
                return Err("Month must be between 1 and 12".to_string());
            }
            return Ok(EventDate::Monthly { year: full_year, month });
        }
    ...
    fn to_formatted_string(&self) -> String {
        match self {
            ...
            EventDate::Monthly { year, month } => format!("00.{:02}.{}", month, year),
            EventDate::Specific(date) => date.format("%d.%m.%Y").to_string(),
        }
    }

    /// Форматирует для отображения пользователю (в списке)
    ...
    /// Возвращает год, если событие имеет годовую привязку (Yearly или Monthly)
    fn get_year(&self) -> Option<i32> {
        match self {
            EventDate::Yearly(y) => Some(*y),
            EventDate::Monthly { year, .. } => Some(*year),
            _ => None,
        }
    }

    /// Вспомогательный конструктор для Monthly (уменьшает дублирование)
    fn monthly(year: i32, month: u32) -> Self {
        EventDate::Monthly { year, month }
    }
}
...
fn get_required_weeks_for_month(year: i32, month: i32, sunday_first: bool) -> usize {
    let first_day = NaiveDate::from_ymd_opt(year, month as u32, 1)
        .expect("Invalid year/month in get_required_weeks_for_month");
...
fn generate_month_lines(
    ...
    let month_name = if is_ru {
        MONTHS_RU[(month - 1) as usize]
    } else {
    ...
    // Дни
    let first_day = NaiveDate::from_ymd_opt(year, month as u32, 1)
        .expect("Invalid year/month in generate_month_lines");
    ...
    let next_month_date = if month == 12 {
        NaiveDate::from_ymd_opt(year + 1, 1, 1).expect("Invalid next month")
    } else {
        NaiveDate::from_ymd_opt(year, (month + 1) as u32, 1).expect("Invalid next month")
    };
    let total_days = next_month_date.signed_duration_since(first_day).num_days() as usize;
    ...
    for day in 1..=total_days {
        let date = NaiveDate::from_ymd_opt(year, month as u32, day as u32)
            .expect("Invalid day in generate_month_lines");
        let wday = date.weekday();
        let is_today = date == today;
        let is_weekend = wday == Weekday::Sat || wday == Weekday::Sun;
        // Проверяем только события на конкретную дату (Specific), Monthly не влияет
        let has_specific_event = show_events && storage.events.contains_key(&EventDate::Specific(date));

        let mut day_str = format!("{:>2}", day);

        if is_today && has_specific_event {
            day_str = format!("{}{}{}{}", INVERT, GREEN, day_str, RESET);
        } else if is_today {
            let color = if is_weekend { RED } else { "" };
            day_str = format!("{}{}{}{}", INVERT, color, day_str, RESET);
        } else if has_specific_event {
    ...
    if !current_line.is_empty() {
        let last_date = NaiveDate::from_ymd_opt(year, month as u32, total_days as u32)
            .expect("Invalid last day in generate_month_lines");
...
fn print_months_row(
    ...
    let block_width = if use_border {
        MONTH_WIDTH_WITH_BORDER
    } else {
        MONTH_WIDTH
    };
    let actual_width = if show_week_numbers {
        block_width + WEEK_NUM_PADDING
    } else {
        block_width
    };
...
fn print_events_list(
    ...
    // 2. Yearly (по годам, сортируем)
    let mut yearly_keys: Vec<&EventDate> = storage.events
        .keys()
        .filter(|k| matches!(k, EventDate::Yearly(_)))
        .collect();
    yearly_keys.sort_by_key(|k| k.get_year().unwrap_or(0));
    for key in yearly_keys {
        if let Some(descs) = storage.events.get(key) {
            for desc in descs {
                ensure_header(&mut header_printed);
                let display = key.to_display_string();
                print_line(&format!("{} - {}", display, desc), interactive);
            }
        }
    }

    // 3. Monthly (выводим только для месяцев, попавших в отображаемый диапазон)
    let mut monthly_entries = Vec::new();
    for &(year, month) in months {
        let key = EventDate::monthly(year, month as u32);
        if let Some(descs) = storage.events.get(&key) {
            for desc in descs {
                monthly_entries.push((year, month, desc.clone()));
            }
        }
    }
    // сортируем по году и месяцу (уже в порядке обхода)
    for (_year, month, desc) in monthly_entries {
        // year не используется – заменяем на _
        ensure_header(&mut header_printed);
        let month_name = if is_ru {
            MONTHS_RU[month as usize - 1]
        } else {
            MONTHS_EN[month as usize - 1]
        };
        // выводим только название месяца, без года
        print_line(&format!("{} - {}", month_name, desc), interactive);
    }

    // 4. Specific (конкретные даты)
    for &(year, month) in months {
        let start_date = NaiveDate::from_ymd_opt(year, month as u32, 1)
            .expect("Invalid start date in print_events_list");
        let end_date = if month == 12 {
            NaiveDate::from_ymd_opt(year + 1, 1, 1)
                .expect("Invalid end date in print_events_list") - Duration::days(1)
        } else {
            NaiveDate::from_ymd_opt(year, (month + 1) as u32, 1)
                .expect("Invalid end date in print_events_list") - Duration::days(1)
        };
...
fn handle_sub_add(storage: &mut EventStorage, is_ru: bool) {
    ...
    if desc == "q" || desc == "Q" || desc.is_empty() {
        return;
    }
...
fn print_help() {
    ...
    println!("  -d          Enable events display. Highlights days with specific events in green and");
    println!("              lists descriptions below the grid for the rendered period.");
    println!("              Supports global '00.00.0000' (always shown), yearly '00.00.YYYY',");
    println!("              monthly '00.MM.YYYY' (shown as 'Month - description'),");
    println!("              and specific 'DD.MM.YYYY' events.");
    ...
    println!("              to edit, supports YY, 00.00.0000, 00.00.YYYY, 00.MM.YYYY), [d] delete event by index.");
            ...
            match c {
                ...
                'c' => {
                    let prev_month_date = NaiveDate::from_ymd_opt(current_year, current_month as u32, 1)
                        .expect("Invalid current month date") - Duration::days(1);
